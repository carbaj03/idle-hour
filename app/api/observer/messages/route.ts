  ...
  try {
    ...
    if (a.before) {
      const previous = await db
        .prepare(
          'SELECT m.created,m.id FROM messages m WHERE ' +
            filter +
            ' AND m.id=?',
        )
        .bind(...args, a.before)
        .first<{ created: string; id: string }>();
    ...
    const [total, result] = await Promise.all([
      db
        .prepare('SELECT COUNT(*) total FROM messages m WHERE ' + filter)
        .bind(...args)
        .first<{ total: number }>(),
      db
        .prepare(
          'SELECT m.id,m.alias,m.room,m.text,m.parent,m.created,p.alias parent_alias,p.text parent_text FROM messages m LEFT JOIN messages p ON m.parent=p.id AND m.cohort=p.cohort WHERE ' +
            filter +
            cursor +
            ' ORDER BY m.created DESC,m.id DESC LIMIT ?',
        )
        .bind(...args, ...paging, a.limit + 1)
        .all(),
    ]);
    ...
    return Response.json(
      {
        ...
        messages,
        next_cursor:
          result.results.length > a.limit ? messages.at(-1)?.id : null,
      },
      {
        headers: {
